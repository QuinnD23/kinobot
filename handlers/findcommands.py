...
async def get_films(check, dp: Dispatcher):
    # количество фильмов
    kol = 1500

    genre_names = ["Боевик", "Мелодрама", "Комедия", "Драма", "Мультфильм", "Триллер", "Ужасы", "Фантастика", "Фэнтези"]
    genre_numbers1 = [3, 7, 6]
    genre_numbers2 = [8, 14, 4]
    genre_numbers3 = [1, 2, 5]
    if check == 1:
        genre_numbers = genre_numbers1
    if check == 2:
        genre_numbers = genre_numbers2
    if check == 3:
        genre_numbers = genre_numbers3
            ...
            while n <= kol and p <= 250:
                ...
                for info in all_info:
                    ...
                    if check_film == -1 and n <= kol:
                        ...
                        n += 1
                await dp.bot.send_message(admin_id, f"Страница: {str(p)}")
                p += 1
