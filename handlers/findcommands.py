...
async def get_rand(user_id, dp: Dispatcher):
    ...
    # количество фильмов
    id_maxi = genre + "$" + min_year + "$" + max_year
    kol = int(await select_db("maxi", "id_maxi", "kol", id_maxi))
...
async def get_films(genre, dp: Dispatcher):
    ...
    for y in range(len(years_min)):
        await dp.bot.send_message(admin_id, f"Начинается загрузка жанра: {genre}")
        ...
        while n <= kol and p <= 250:
            ...
            for info in all_info:
                ...
                if check_film == -1 and n <= kol:
                    ...
                    n += 1
            await dp.bot.send_message(admin_id, f"Страница: {str(p)} - {str(n-1)}/2000")
            p += 1

        # сохранение максимума для рандома
        id_maxi = genre + "$" + min_year + "$" + max_year
        await insert_db("maxi", "id_maxi", id_maxi)
        await update_db("maxi", "id_maxi", "genre", id_maxi, genre)
        await update_db("maxi", "id_maxi", "min_year", id_maxi, min_year)
        await update_db("maxi", "id_maxi", "max_year", id_maxi, max_year)
        n -= 1
        await update_db("maxi", "id_maxi", "kol", id_maxi, n)

    await dp.bot.send_message(admin_id, f"Загрузка жанра окончена: {genre}'")
